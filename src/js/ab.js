 ...
 */
ab('optin', 0.5).slices('optin-checked').run(function () {
    var test = this.name;
    var slice = this.slice.name;

    if (slice !== 'control') {
        var variation = 'tests/' + test + '/variations/' + slice + '.js';
        this.script(variation);
    }

    // document.querySelector('.current-variation').textContent = slice;
});
